...
export function VestingSchedule({ positions }: Props) {
  ...
  const formatTimeLeft = (ms: number) => {
    ...
    if (diff <= 0) return t('vestingComplete');
    const d = Math.floor(diff / 86400000);
    const h = Math.floor((diff % 86400000) / 3600000);
    if (d > 0) return t('dRemaining', { d, h });
    return t('hRemaining', { h });
  };

  const handleClaim = (symbol: string, amount: number) => {
    toast.success(`${t('claimBtn')} ${amount.toLocaleString()} ${symbol}!`, {
    ...
    <div className="bg-bg-075 border border-white/10 rounded-[14px] p-6 mb-5">
      <div className="flex items-center justify-between mb-5">
        <div className="flex items-center gap-2.5">
          ...
          <span className="font-[family-name:var(--font-display)] text-[1.1rem] font-bold">{t('title')}</span>
        </div>
        <span className="text-[0.78rem] text-white/40 font-[family-name:var(--font-mono)]">
          {t('claimCount', { count: withVesting.filter(p => p.claimable > 0).length })}
        </span>
      </div>
      ...
      <div className="space-y-4">
        {withVesting.map((pos) => {
          ...
          const milestones = [
            { label: t('tge'), pct: 25, done: true },
            { label: t('month1'), pct: 50, done: progressPct >= 50 },
            { label: t('month2'), pct: 75, done: progressPct >= 75 },
            { label: t('fullVest'), pct: 100, done: isComplete },
          ];
            ...
            <div key={pos.id} className="rounded-[12px] border border-white/8 bg-white/[0.015] p-4">
              ...
              <div className="grid grid-cols-3 gap-2 mb-3">
                <div className="text-center">
                  <div className="text-[0.68rem] text-white/40 uppercase tracking-wider mb-0.5">{t('claimable')}</div>
                  <div className={cn(
                ...
                <div className="text-center">
                  <div className="text-[0.68rem] text-white/40 uppercase tracking-wider mb-0.5">{t('progress')}</div>
                  <div className="font-[family-name:var(--font-mono)] font-bold text-[0.86rem]">
                    {Math.round(progressPct)}%
                  </div>
                </div>
                <div className="text-center">
                  <div className="text-[0.68rem] text-white/40 uppercase tracking-wider mb-0.5">{t('vestsBy')}</div>
                  <div className="font-[family-name:var(--font-mono)] text-[0.75rem] text-white/50">
                    {pos.vestingEnds ? (isComplete ? t('completed') : formatDate(pos.vestingEnds)) : '—'}
                  </div>
                </div>
              </div>
              ...
              {hasClaim && (
                ...
                >
                  ...
                  {t('claimBtn')} {pos.claimable.toLocaleString()} {pos.symbol}
